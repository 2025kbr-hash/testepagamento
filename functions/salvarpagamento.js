...
headers });
  if (method !== "POST") return new Response("Método não permitido", 
{ status: 405, headers });
  ...
  try {
    ...
    if (!SUP_URL || !SUP_KEY) {
      return new Response("Variáveis de ambiente ausentes.", { 
status: 500, headers });
    }
    ...
    return new Response(`Supabase retornou: 
${res.status}\\n${text}`, {
      status: res.ok ? 200 : 500,
      headers,
    });
  } catch (err) {
    return new Response("Erro interno: " + err.message, { status: 
500, headers });
  }
}
export 
